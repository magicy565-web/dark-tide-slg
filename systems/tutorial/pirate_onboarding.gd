...
const STEPS: Array = [
	...
	# ─────────────────────────────────────────────────────────────────────────
	{
		...
		"phase": "harbor",
		"title": "【港口】海上战略核心",
		"text":"[color=cyan]港口[/color]是海盗阵营的战略核心，占领港口将解锁多项重要功能：\n\n[color=yellow]港口的价值：[/color]\n• 触发剧情支线「[color=gold]海盗宝藏[/color]」（奖励 800 金 + 掠夺 +20）\n• 满足挑战任务「恐怖航路」（pirate_c2）的条件\n• 增加性奴隶容量上限（+3）\n• 港口格子每回合额外金币收入\n\n[color=orange]在地图上找到港口格子（HARBOR 类型），发动攻击并占领它！[/color]",
		"trigger": "pirate_hero_recruited",
		"highlight": "",
	},
	{
...
func _on_combat_result(player_id: int, _target_desc: String, won: bool) -> void:
